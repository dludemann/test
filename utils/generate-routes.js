...
const contentDirectories = ['./content/tinder', './content/dil-mil', './content/bumble', './content/hinge'];

// Get the routes from the content directories
const routes = contentDirectories.flatMap((dir) => {
  return fs.readdirSync(dir).map((file) => {
    return '/' + path.join(dir, path.basename(file, path.extname(file))).replace('content/', '');
  });
});
          ...
          property.value.properties.forEach((subProperty) => {
            if (subProperty.key.name === 'routes') {
              routes.forEach((route) => {
                ...
                if (!subProperty.value.elements.some((element) => element.value === route)) {
